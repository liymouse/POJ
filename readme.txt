...
1445  2次DP（第一次从前往后算出总数T，第二次从后往前算出开头为i的个数）+高精度（模板）
1446  时区变换 模拟
1447  日期处理(注意年份位数只能为1,2,4，日月位数只能为1,2)
1448  6个面拼接成立方体,dfs
1451  字典树（模拟T9输入法）
1453  模拟，字符串处理
1457  搜索,一个一个搜索即可，数据弱
1460  算式计算,表达式树
1464  树形dp
1465  BFS，利用同余剪枝，每个余数只取最小数的那个即可
1466  最大独立点集(=N-最大匹配/2)
1472  递归
1474  一个点可以看到多边形全部区域，半平面交（模板）
1476  DP f[t][i] = min(f[t-1][j] + v[i][j][(t-1)%v[i][j].size()])
1478  枚举，注意输出，能确定一部分就输出一部分
1479  Floyd  编程基本功
1480  搜索(BFS+DFS)
1481  DFS(2种联通区域)
1482  SPFA (通过位来做状态转移）
...
1484  模拟
1485  dp
1486+ 贪心是错的但是能ac（每次找字母或者数字中度数为1的点，然后删去），正确算法是二分图匹配，并找到关键匹配
1489  字符串+模拟，注意需要用longlong，超过99999的数只处理低5位
1492  计数模拟
1495  模拟+枚举
1497  搜索+剪枝
1498  模拟，注意周末不能安排会议
1506  拓扑排序，注意字符串个数比较多
1508  并查集+扩展gcd求方程解
1510-
1513  DP f[i][j] = min(f[k][j-1] + c) f[k][j-1]有效，且sum[k+1 ~ i] <= L
1703  并查集
1742  DP
1789  最小生成树prim
1797  dijkstra
